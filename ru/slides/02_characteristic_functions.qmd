...
::: {.incremental}
- Распределение Коши ($\mathrm{Cauchy}(x_0,\gamma)$):
$$
f_X(x;x_0,\gamma)=\frac{1}{\pi\gamma\left[1+\left(\frac{x-x_0}{\gamma}\right)^2\right]}.
$$
...
- Стандартное распределение Коши: $x_0=0$, $\gamma=1$ ($\mathrm{Cauchy}(0,1)$):
$$
...
- Пусть $X,Y\sim\mathrm{Cauchy}(0,1)$ независимы:
$$
f_X(x)=f_Y(x)=\frac{1}{\pi(1+x^2)}.
$$

- Для суммы $S=X+Y$ прямая свёртка имеет вид
$$
\boxed{
f_S(s)
=
...
Простая постановка задачи привела к громоздкому интегралу.
:::
...
## Новый язык: характеристическая функция
...
- Если плотность существует, её можно восстановить:
$$
f_X(x)
=
\frac{1}{2\pi}
...
- Для стандартного распределения Коши Фурье-образ имеет вид:
$$
\varphi_X(t)=e^{-|t|}.
$$

- Независимость превращает сумму в произведение:
$$
\begin{aligned}
\varphi_S(t)
&=
...
- Ответ получаем без вычисления свёртки:
$$
\boxed{
\begin{aligned}
...
1. **Преобразовать обе плотности:**
   ...
   $$
2. **Заменить сумму произведением:**
   $$
   \varphi_S(t)
   =
   \varphi_X(t)\varphi_Y(t).
   $$
3. **Вернуться к плотности:** и вычислить последний интеграл
   $$
   f_S(s)
   =
   \frac{1}{2\pi}
...
Если случайная величина принимает значения $x_k$ с вероятностями $p_k$, то
$$
P(X=x_k)=p_k,
\qquad
\sum_k p_k=1,
...
::: incremental
- Значение при $t=0$ выражает нормировку вероятности:
$$
...
В примере с распределением Коши мы доказали, что для двух независимых величин
$$
\varphi_{X+Y}(t)=\varphi_X(t)\varphi_Y(t).
$$

Последовательно применяя это равенство к
$$
S_n=X_1+\dots+X_n,
$$
получаем
...
Для одинаково распределённых величин:
$$
\boxed{
\varphi_{S_n}(t)
=
...
- Для суммы $n$ независимых величин Бернулли:
$$
\varphi_{S_n}(t)
=
...
S_n=\sum_{k=1}^n N_k
$$
пуассоновских величин:
$$
\varphi_{S_n}(t)
=
\prod_{k=1}^n\varphi_{N_k}(t) = \prod_{k=1}^n e^{\lambda_k\left(e^{it}-1\right)}=e^{\lambda\left(e^{it}-1\right)},
$$
где 
$$
...
- Их параметры $\lambda$ складываются.
:::
...
- Тогда характеристическая функция суммы $S_n=\sum_{k=1}^n X_k$ равна 
$$
\begin{aligned}
\varphi_{S_n}(t)
&=
...
\end{aligned}
$$
- Сумма независимых гауссовых величин распределена по гауссу:
$$
\boxed{
S_n
\sim
...
::: incremental
- Для независимых $X_k\sim\mathrm{Cauchy}(0,1)$:
$$
\varphi_{S_n}(t)
=
\left(e^{-|t|}\right)^n
...
Если соответствующие моменты существуют, то
$$
\varphi_X(t)
=
\mathbb{E}\!\left[e^{itX}\right]
...
- Она связана с характеристической функцией формальной заменой $t\to -it$.
- В отличие от $\varphi_X(t)$, функция $M_X(t)$ может не существовать при $t\ne0$.
- Для общих рассуждений характеристическая функция надёжнее.
:::
...
Из коэффициентов ряда их можно извлечь производными:
$$
\boxed{
\kappa_n
=
...
Делим на соответствующие степени $i$:
$$
\boxed{
...
Для центрированной величины $Y=X-\mu$ обозначим
$$
m_r=\mathbb E[Y^r],
\qquad
m_2=\sigma^2.
$$
Поскольку $K_Y(t)=K_X(t)-i\mu t$, центрирование изменяет только первую кумулянту.

Её характеристическая функция около нуля:
$$
\varphi_Y(t)
=
1-\frac{m_2t^2}{2}
...
Используя $\ln(1+u)=u-u^2/2+\cdots$, получаем
$$
K_Y(t)
=
-\frac{m_2t^2}{2}
...
Сравнивая коэффициенты при степенях $it$, находим
$$
\boxed{
\begin{aligned}
\kappa_1&=\mu,\\
...
Пусть одинаково распределённые независимые величины имеют
$$
\mathbb{E}[X_i]=\mu,
\qquad
...
Если среднее равно нулю, а дисперсия равна единице, то при малых $u$
$$
\varphi_Y(u)
=
1-\frac{u^2}{2}+o(u^2).
...
e^{-t^2/2}
$$
— характеристическая функция стандартного гауссова распределения.

## Смысл результата

::: {.callout-important .thmbox appearance="simple"}
## Почему возникает гаусс

После центрирования и нормировки высшие детали распределения отдельных вкладов постепенно исчезают.

В пределе остаётся универсальная квадратичная зависимость
$$
\ln\varphi(t)\simeq-\frac{t^2}{2}.
$$
:::
...
\sum_{k=1}^n(X_k-\mu)
$$
нормированные кумулянты ведут себя как
$$
\kappa_r(Z_n)
\propto
n^{1-r/2}.
