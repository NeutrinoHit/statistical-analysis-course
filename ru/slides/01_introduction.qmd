...
::: {.centered-figure style="--figure-width: 132%; --figure-max-height: 110vh;"}
![](shared/figures/bayes-theorem.svg){fig-alt="Иллюстрация теоремы Байеса"}
:::

## Литература

- Particle Data Group: [Review of statistics (2026)](http://pdg.lbl.gov/2026/reviews/rpp2026-rev-statistics.pdf)
...
![](shared/figures/DayaBayTheta13.png){width=100%}
...
![](shared/figures/DBSterile.png){width=75%}
...
## Псевдоданные и фит {#applet-gaussian-fit}
...
curveX = Array.from({length: 401}, (_, i) => xMin + (xMax - xMin) * i / 400)
...
normCurve = (s) => {
  const w = curveX.map((xi) => gaussShape(xi, s));
  const wMax = Math.max(...w, 1e-12);
  return curveX.map((xi, i) => ({x: xi, y: w[i] / wMax}));
}
...
sigmaGrid = Array.from({length: 681}, (_, i) => 0.30 + 0.0025 * i)
...
<style>
.fit-widget {
  --fit-main-width: 820px;
  --fit-profile-width: 620px;
  --fit-profile-shift: 2.1rem;
}
...
.fit-widget .fit-layout {
  display: grid;
  grid-template-columns: var(--fit-main-width) var(--fit-profile-width);
  gap: 0.35rem;
  width: max-content;
  max-width: 100%;
  align-items: start;
...
.fit-widget .fit-side {
  transform: translateX(var(--fit-profile-shift));
}
